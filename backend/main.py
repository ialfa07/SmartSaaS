...
def verify_payment_endpoint(session_id: str, user: User = Depends(fake_auth)):
    ...
    try:
        ...
        if payment_info["status"] == "paid":
            ...
            return {
                "success": True,
                "credits": users_db[user.email]["credits"]
            }
        else:
...
def generate_calendar_endpoint(request: CalendarRequest,
    ...
    if users_db[user.email]["credits"] < 10:  # 10 crédits pour calendrier
        raise HTTPException(status_code=403,
                            detail="Not enough credits (10 required)")

    result = generate_content_calendar(request.business_type,
...
def claim_daily_reward(user: User = Depends(fake_auth)):
    ...
    tokens_data = add_tokens(user.email, TOKEN_REWARDS["daily_login"],
                             "daily_login")
    ...
    return {
        ...
        "message":
        f"Vous avez gagné {TOKEN_REWARDS['daily_login']} jetons SaaS !"
    }
...
def get_referral_data(user: User = Depends(fake_auth)):
    ...
    return {
        ...
        "referral_link":
        f"https://smartsaas.com/signup?ref={referral_data['referral_code']}",
        "rewards": {
...
def refer_user(referral: ReferralRequest, user: User = Depends(fake_auth)):
    ...
    if result["success"]:
        return {
            "success": True,
            "message":
            f"Parrainage réussi ! Vous avez gagné {result['referrer_reward']} jetons.",
            "referrer_reward": result["referrer_reward"],
            "new_balance": result["referrer_balance"]
        }
    else:
        raise HTTPException(status_code=400,
                            detail="Erreur lors du parrainage")
...
def exchange_tokens_for_credits(amount: int, user: User = Depends(fake_auth)):
    ...
    if spend_tokens(user.email, amount,
                    f"exchange_for_{credits_to_add}_credits"):
